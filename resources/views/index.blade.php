        ...
        document.getElementById('priceForm').addEventListener('submit', function(event) {  
            ...
            .then(data => {  
                ...
                Object.keys(data).forEach(date => {  
                    const roundedPrice = Math.round(data[date]); // Membulatkan harga  
                    const formattedPrice = formatRupiah(roundedPrice); // Memformat harga  
                    const row = document.createElement('tr');  
                    row.innerHTML = `  
                        <td>${date}</td>  
                        <td>Rp ${formattedPrice}</td> <!-- Tampilkan harga yang sudah diformat -->  
                    `;  
                    priceData.appendChild(row);  
  
                    // Menyimpan data untuk diagram  
                    labels.push(date);  
                    prices.push(roundedPrice); // Simpan harga yang sudah dibulatkan  
                });  
